...
# Re-install 0xProto pour la police
function fontscheck{
    $fontName = "0xProto"
    
    # Verifie si la police est install
    $fonts = Get-WmiObject -Query "SELECT * FROM Win32_FontInfo" 2>$null
    $fontInstalled = $fonts | Where-Object { $_.Name -eq $fontName }

    if ($fontInstalled) {
        Write-Host "$fontName d�j� install�"
    } else {
        Install-Module -Name oh-my-posh -Force -Scope CurrentUser
        oh-my-posh font install $fontName
        Write-Host "$fontName a �t� install�"
    }
}
...
function Update-Profile {
    ...
    try {
        $url = "https://raw.githubusercontent.com/AtisFPS/Powerterm/main/Microsoft.PowerShell_profile.ps1"
        ...
        Write-Host "Erreur de profile TEMP ..."
        Write-Host "MàJ du profile Powershell cancel ..."
...
# Open ToolsBox
function tools {
	irm "https://raw.githubusercontent.com/AtisFPS/ToolsBox/refs/heads/main/tools.ps1" | iex
...
# Get the Powershell command history
function history {
	Get-History
}
...
tools - Run the ToolsBox script from Hugo SERRURE .
